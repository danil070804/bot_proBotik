...
def _guide_text():
	return (
		...
		'  4) если задан <code>ACCOUNT_CHECK_CHAT</code> — проверка доступа к тест-чату.\n'
		'• Статусы аккаунта:\n'
		...
		'• Все статусы хранятся в БД (таблица <code>account_health</code>), есть ручная кнопка «Проверить аккаунты» и фоновый монитор (интервал <code>ACCOUNT_MONITOR_INTERVAL</code>).\n'
		...
		'🔐 <b>Обязательные переменные</b>\n'
		'• <code>TG_API_ID</code>, <code>TG_API_HASH</code> — обязательны для проверки и работы сессий.\n'
		'• <code>ACCOUNT_CHECK_CHAT</code> — опционально, тест-чат для проверки доступа.\n'
		'• <code>ACCOUNT_MONITOR_INTERVAL</code> — интервал фоновой проверки в секундах.\n\n'
